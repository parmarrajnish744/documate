...
const OperatorLoginPage: React.FC<OperatorLoginPageProps> = ({ onLogin, users, onBack }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        setError('');
        setTimeout(() => {
            const user = users.find(u => u.email.toLowerCase() === email.toLowerCase() && u.role === 'operator');
            if (user) {
                onLogin(user);
            } else {
    ...
    return (
        <div className="max-w-md mx-auto bg-white dark:bg-gray-800 p-8 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700">
            ...
            <h2 className="text-2xl font-bold text-center mb-6 dark:text-gray-100">{t('operator.loginTitle')}</h2>
